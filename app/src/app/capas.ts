 ...
 */
const PORTALES = '/data/2026-05-13_zgzradar_callejero-portales-zaragoza.json';
 ...
 */
const GRAFO = '/data/grafo-visor.js';
 ...
 * También en [lon, lat].
 */
const CARRILES = '/data/2026-08-04_wfs_movilidad-MU2_carriles_bici.json';
 ...
 */
const POSTES = '/data/2026-08-10_wfs_movilidad-MU3_paradas_bus_unicas.json';
 ...
 */
const TRAZADOS = '/data/2026-08-10_nap_gtfs-ficha1176_shapes.txt';
 ...
 */
const PARADAS = '/data/2026-08-10_nap_gtfs-ficha1176_stops.txt';
...
const BIZI = [0, 50, 100, 150, 200, 250].map(
  (p) => `/data/2026-08-02_wfs_bizi_pag${p}.json`,
);

/** ANDAMIO DE VERIFICACIÓN. Los 2.158 aparcabicis, descarga propia del WFS. */
const APARCABICIS = '/data/2026-08-17_wfs_movilidad-MU2_aparcabicis.json';
 ...
 */
const APARCAMOTOS = '/data/2026-08-18_wfs_movilidad-MU2_motos.json';

/**
 * ANDAMIO DE VERIFICACIÓN. El estacionamiento regulado en superficie, tercera
 * descarga propia: 7.424 tramos de bordillo (MultiLineString).
 *
 * **De los 7.424 solo se pintan 1.177.** El campo que manda es `tipo_actual`:
 * `ESRO` es rotación —la zona azul— y `ESRE` residentes; los 6.217 `LIBRE` y
 * los 30 sin clasificar **no se pintan**, porque no son regulado. Viajan en el
 * fichero, que se copió entero y tal cual.
 *
 * ⚠️ **`zona_reguladora` NO dice si está regulado**: es un perímetro
 * geográfico, y **5.059 tramos LIBRES lo llevan**. Filtrar por ahí pintaría de
 * pago 5.059 bordillos gratuitos. Ver THIRD-PARTY-NOTICES § 1.11.
 */
const REGULADO = '/data/2026-08-18_wfs_movilidad-MU1_estacionamientos_calle.json';
 ...
 * Y son exactamente las que tienen tramos de pago — 1.173 de los 1.177.
 */
 ...
 */
const ZONAS = '/data/2026-08-18_wfs_movilidad-MU1_zonas_reguladas.json';

/**
 * ANDAMIO DE VERIFICACIÓN. El censo de reservas de espacio, quinta descarga
 * propia: 2.656 puntos de los que **solo se pintan las 1.236 PMR**. El resto
 * —carga y descarga, taxi, sanitarias…— viaja en el fichero sin pintarse.
 *
 * ⚠️ **El campo que manda es `TIPO`, no `SUBTIPO`.** 1.395 registros llevan
 ...
 */
const RESERVAS = '/data/2026-08-18_wfs_movilidad-MU1_reservas.json';
...
export class Capas implements CapasDeVerificacion {
   ...
   * 26, 27, 29, 32, 33, 34, 37, 39, 40, 43, 46 y 47), con 21.273 plazas.
   *
   * Por qué existe: esos 19 números **no cobran ni uno solo de sus tramos**,
   * mientras que las 13 zonas con polígono se llevan 1.173 de los 1.177 de
  ...
  /**
   * Las 1.236 reservas PMR **en vigor**. Las retiradas y las denegadas se
