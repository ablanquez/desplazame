...
**Nota [2026-09-17] · CORRIGE LA ANTERIOR:** la nota de arriba era falsa. Ni la contraprueba imprimía su pid ni el perfil reapareció. `perfil-medir-9350-18164` se creó a las 14:48, y la contraprueba corrió antes de las 14:21. Es de **`proximo-bus`**: al parar la batería con `TaskStop` **no murió el guion `sh`**, que siguió solo con `pintura`, `proximo-bus` y `yego`. `proximo-bus` lo dejó escrito: `⚠️  NO se ha podido borrar …/perfil-medir-9350-18164`. Cifra corregida: **10 cierres fallidos, todos en suites reales**: `bizi-y-resumen` 1, `dos-filas` 1, `identidad` 1, `pantalla` 1, `pintura` 5, `proximo-bus` 1. `yego` salió en rojo porque yo ya había parado el motor.
